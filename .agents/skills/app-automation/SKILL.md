...
## When to use
- iOS Simulator 또는 macOS 앱을 실제로 조작·검증해야 할 때
- selector 기반 UI 자동화, 스크린샷, 비디오 같은 실행 증거가 필요할 때
- 멀티스텝 시나리오를 재현 가능하게 남겨야 할 때

## Do not use when
- 일반 앱 개발 질문이나 코드 작성만 필요한 작업
- 브라우저 웹 자동화가 주목적인 작업
- 수동 설명만 있으면 충분하고 실행 증거가 필요 없는 작업

## Quick start
1. `doctor` 또는 `baepsae_version`으로 환경을 확인한다.
2. 대상이 simulator인지 macOS 앱인지 먼저 정한다.
3. `analyze_ui`/`query_ui`로 상태를 읽은 뒤 selector 기반으로 조작한다.
4. `run_steps`와 screenshot/video로 evidence를 남긴다.
