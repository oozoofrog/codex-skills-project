...
## Do not use when
- 새 스킬을 만들거나 구조를 재작성해야 하는 작업 → `codex-skill-bootstrap`
- plugin packaging, marketplace, legacy residue 감사가 주목적인 작업 → `plugin-doctor`
- 저장소 전체가 아니라 instruction 계층 자체를 다루는 작업 → `agent-context-*`

## Quick start
1. `audit_codex_skill_repo.py [target]`로 결정적 감사를 먼저 수행한다.
2. trigger boundary, references 분리, review harness, metadata를 수동 점검한다.
3. `critical / warning / info / strength`로 정리하고 바로 고칠 파일부터 제안한다.

## Use these resources

- `references/audit-checklist.md` — 사람이 직접 볼 감사 기준
- `scripts/audit_codex_skill_repo.py` — 반복 가능한 구조 점검 스크립트
- `schemas/report.schema.json` — machine summary JSON shape
- `../../../docs/evaluator-output-contract.md` — evaluator 출력 공통 계약
...
- `When to use`, `Do not use when`, `Quick start`, `Output expectation`이 operator 관점에서 충분히 명확한가
