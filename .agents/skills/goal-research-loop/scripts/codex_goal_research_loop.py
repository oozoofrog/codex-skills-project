#!/usr/bin/env python3
"""Host-managed Codex CLI research loop runner for the goal-research-loop skill."""
...
SCHEMA_PATH = ROOT / "schemas" / "round-result.schema.json"
DEFAULT_STATE_DIR_NAME = ".goal-research-loop"
LEDGER_HEADER = (
...
def sanitize_status_lines(lines: Iterable[str], state_dir_rel: Path | None) -> list[str]:
    filtered: list[str] = []
...
def restore_workspace(workspace: Path, state_dir_rel: Path | None) -> None:
    ...
    run_subprocess(clean_cmd, cwd=workspace)
...
def commit_keep_result(
    ...
    message = f"goal-research-loop round {round_num:03d}: {short_hypothesis}"
...
def build_round_prompt(
    *,
    ...
    if extra_instructions:
        extra_text = "\n".join(f"- {collapse_ws(item)}" for item in extra_instructions)
    else:
        extra_text = "- 추가 지시 없음"
...
def build_codex_command(
    *,
    ...
    cmd.extend(["exec", "-", "-C", str(workspace), "--output-schema", str(SCHEMA_PATH), "--json", "-o", str(last_message_path)])
    ...
    add_dirs: list[Path] = []
    if not is_relative_to(skill_dir, workspace):
        add_dirs.append(skill_dir)
    if not is_relative_to(state_dir, workspace):
...
def maybe_bootstrap_files(workspace: Path, state_dir: Path, objective: str | None, force: bool) -> list[Path]:
...
def should_stop(control_action: str, loop_forever: bool, rounds_done: int, max_rounds: int | None) -> bool:
...
def cmd_run(args: argparse.Namespace) -> int:
    ...
    state_dir_rel = relative_to_or_none(state_dir, workspace)
    ...
    while True:
        ...
        if git_root:
            ...
            elif experiment_status in {"discard", "crash"} and workspace_changes_exist(workspace, state_dir_rel):
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="goal-research-loop 스킬용 Codex CLI 반복 실행기",
    )
    subparsers = parser.add_subparsers(dest="command", required=True)

    init_parser = subparsers.add_parser("init", help="program/contract/state/ledger 템플릿 생성")
    init_parser.add_argument("--workspace", default=".", help="연구를 수행할 workspace 디렉터리")
    init_parser.add_argument("--state-dir", help="상태 파일을 둘 디렉터리 (기본: <workspace>/.goal-research-loop)")
    init_parser.add_argument("--objective", help="초기 objective 문장")
    init_parser.add_argument("--force", action="store_true", help="기존 템플릿 파일도 덮어쓰기")
    init_parser.set_defaults(func=cmd_init)

    status_parser = subparsers.add_parser("status", help="현재 snapshot/ledger 상태 출력")
    ...
    run_parser.add_argument("--state-dir", help="상태 파일 디렉터리")
    run_parser.add_argument("--objective", help="state dir가 아직 없을 때만 초기 objective로 사용")
    run_parser.add_argument("--codex-bin", default="codex", help="Codex CLI 실행 파일 경로")
    run_parser.add_argument("--model", help="codex exec 에 전달할 model")
    run_parser.add_argument("--profile", help="codex exec 에 전달할 profile")
    run_parser.add_argument("--sandbox", choices=["read-only", "workspace-write", "danger-full-access"], help="codex exec sandbox")
    run_parser.add_argument("--search", action="store_true", help="Codex web search 활성화")
    run_parser.add_argument("--full-auto", action="store_true", help="codex exec --full-auto 전달")
    run_parser.add_argument(
        "--dangerously-bypass-approvals-and-sandbox",
        action="store_true",
        help="codex exec --dangerously-bypass-approvals-and-sandbox 전달",
    )
    run_parser.add_argument("--skip-git-repo-check", action="store_true", help="codex exec --skip-git-repo-check 전달")
    run_parser.add_argument("--max-rounds", type=int, default=3, help="최대 라운드 수 (기본: 3)")
    run_parser.add_argument("--loop-forever", action="store_true", help="control_action이 종료 신호를 줄 때까지 계속 실행")
