...
## Do not use when
- 구조 생성 없이 repo-wide 품질 감사만 필요한 작업 → `codex-skill-audit`
- plugin packaging 구조를 다뤄야 하는 작업 → `plugin-doctor`
- 작은 문구 수정처럼 skill 구조 재정비가 필요 없는 작업

## Quick start
1. 대상 스킬의 범위와 trigger를 먼저 정한다.
2. 최소 구조(`SKILL.md`, 필요 시 `agents/openai.yaml`)를 만든다.
3. `When to use / Do not use when / Quick start / Review Harness / Output expectation` 중심으로 본문을 정리한다.
4. 마지막에 `codex-skill-audit`로 discovery와 metadata를 검증한다.
...
- `When to use / Do not use when / Quick start / Output expectation`이 있는지
- 스킬 이름이 중복되지 않는지
